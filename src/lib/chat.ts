...
import type { Strategy, Candle, BacktestResult } from './trading';
...
class ChatService {
  ...
  async sendWithContext(
    ...
    const context = `
      ...
      - ${backtestResult ? `Net Profit: ${backtestResult.metrics.netProfit.toFixed(2)}, Win Rate: ${(backtestResult.metrics.winRate * 100).toFixed(2)}%, Trades: ${backtestResult.metrics.totalTrades}` : 'No backtest run yet.'}
      User Query: ${message}
    `;
