...
export function ChatAssistantWrapper({ strategy, backtestResult }: ChatAssistantWrapperProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await chatService.sendWithContext(message, strategy, backtestResult, chatState.model, (chunk) => {
      setChatState(prev => ({
        ...prev,
              ...
              <div className="flex justify-start">
                <div className="bg-muted p-3 rounded-2xl">
                  <div className="flex space-x-1">
                    {[0, 1, 2].map(i => (
                      <div key={i} className="w-2 h-2 bg-current rounded-full animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
                    ))}
                  </div>
                </div>
              </div>
      ...
      <CardFooter className="p-4 border-t">
        <form onSubmit={handleSubmit} className="w-full flex gap-2 items-center">
          <Textarea
            ...
            placeholder="Ask AI to explain a signal..."
